...
s = L.setHead(s, {
  ...
  desc: 'Technical, local and content SEO plus AEO/GEO for US small and mid-size businesses &mdash; built on real search data, reported in plain English every month. One agreed monthly fee, no long-term contract, no paid links.',
  url: URL,
...
const faqs = [
  ...
  { q: 'Do you use AI to write our content?', a: 'We use AI as a research and drafting accelerant, and a senior engineer fact-checks, edits and takes responsibility for every page before it goes live. Thin auto-generated content is exactly what search engines are demoting, so we do not publish it.' },
  ...
  { q: 'Should we hire a US agency or an offshore team?', a: 'The honest trade-offs are time zone and communication. Our engineers work from Kochi, India, with a live overlap window every US morning, same-day written responses, a weekly call and a engineering team that knows your account &mdash; and you keep 40&ndash;60% of the typical US agency fee. If you need someone in your office, we are not the right fit.' },
...
let body = '';
body += L.answer('<strong>SEO services</strong> are the ongoing work of making your website easy for Google &mdash; and now ChatGPT, Perplexity and Google’s AI Overviews &mdash; to find, understand and trust enough to recommend. TechAuditPros delivers the full program for US businesses: a real audit first, then technical fixes, content, local search and AI-citation structuring, run by <strong>one dedicated engineering team</strong> against real US keyword data and reported monthly, for a <strong>one agreed monthly fee</strong> instead of the US$3,000&ndash;$5,000+ retainers common at US agencies.');
...
body += L.promise({
  ...
  p: 'Before we recommend anything, your dedicated engineering team goes through what is actually on your site &mdash; what ranks, what is thin, what is technically broken, and which US searches you are missing entirely &mdash; and walks you through the findings. The plan that follows is built on your gaps and your market, not a package every client gets.',
...
body += L.processRow('process', 'How We Work', 'A 6-step program, from audit to compounding results.', [
  ...
  { h4: 'Report &amp; Iterate', p: 'A plain-English monthly report and the next month’s plan, from the same team that ran the audit.' },
]);
...
body += L.costGrid('seo-cost', 'Pricing', 'What SEO services cost in the US &mdash; and what you get for it.', 'Transparent numbers, because you should be able to compare before you talk to anyone.', [
  ...
  { tag: 'TechAuditPros', num: 'Let’s talk', sub: 'Scoped on the call and put in writing', hi: true, items: ['Audit, technical fixes, content, local, AEO/GEO and reporting included', 'A engineering team that does the work and answers your questions', 'Fixes implemented directly, not ticketed', 'Plain-English monthly report', 'Stop any time; everything built stays on your site'] },
