...
<style>
  ...
  /* full-bleed band, edge to edge on any desktop (owner: full size width, full desktop) */
  .final-cta-section{ position:relative; overflow:hidden; padding:0 !important; background:var(--navy-deep); }
  .final-cta-section > .container{ position:relative; z-index:2; max-width:none; padding:0; }
  .final-cta{ background:transparent !important; border-radius:0; max-width:1180px; margin:0 auto; padding:104px 40px; }
  @media (min-width:1440px){ .final-cta{ max-width:1480px; } }
  @media (min-width:1850px){ .final-cta{ max-width:1720px; } }
  @media (max-width:760px){ .final-cta{ padding:64px 24px; } }
  ...
  html.hm .final-cta-section.hm-live::before{ animation:hmAurora 30s ease-in-out infinite alternate; }
  ...
  html.hm .final-cta-section.hm-live .hm-vib{ animation:hmSheen 14s linear infinite; }
...
(function(){
    ...
    var band = document.querySelector('.final-cta-section');
    ...
    if (band && cta && document.createElement('canvas').getContext){
        ...
        function layout(){
            ...
            var cr = band.getBoundingClientRect();
        ...
        new IntersectionObserver(function(es){
            ...
            band.classList.toggle('hm-live', vis);             // the two loops only exist while it is on screen
