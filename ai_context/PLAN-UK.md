...
## 6c. Copy rules the owner set on 2026-09-03 (apply to every page from here)
- **No single-engineer promise.** Never "a dedicated engineer", "your engineer", "a named engineer". It is a team: "a dedicated engineering team", "one accountable team", "the same team".
- **Never publish our own price.** No £1,200 / US$1,800 / CA$1,490 / AU$1,600 / AED 3,800, and no "save 57%" claims. Use "one agreed monthly fee", "Let’s talk", "scoped on a call, in writing". Third-party market prices (Sage 200 £374/mo, NetSuite £74/user, agency retainer ranges) STAY — they are the research that makes "let’s talk about yours" read informed rather than evasive.
- **Every page independent, in design and in content** (owner, 2026-09-03: "no duplicate content or sections, each page its own independent design and content"). No section may be byte-identical to one on another page, and each page gets its own section vocabulary and its own signature animation. The only deliberate repeat is the closing CTA band, which is a call to action rather than content. Audit before shipping: hash every section block across the site and fail on any match. Current state: 0 shared blocks between /uk/, /uk/erp/ and /uk/website-development/, and their section sets are disjoint apart from process/contact.
- **No repeated boilerplate sections.** The "Why TechAuditPros / your data stays yours" badge + "Built on" stack block is deleted and must not be re-added; its trust facts live in each page’s FAQ and, on /uk/erp/, in the UK compliance section. Before shipping a page, diff its sections against the sibling pages.
- Enforcement: run `node scripts/country-pages/tone_pass.js` after every generator run. It has a fixed-point guard (no rule may match its own output) and never rewrites L.must assertions, so generator self-checks must be updated by hand when the copy they guard changes.

## 6d. Shared components that are allowed to repeat (owner-requested)
Two elements appear on many pages on purpose, and are UI rather than content, so they do not breach the no-duplicate rule: the closing converging-lines CTA band, and the big moving stack strip (trust_logos.js, 26 tool names, 14 pages). The strip uses wordmarks, not logo artwork - the only logo SVGs in the repo are generic placeholder glyphs, and drawing brand marks from memory would be inaccurate and a trademark risk. It is captioned "built on and worked in daily", never as partnership or endorsement. To swap in an official mark: drop the SVG in assets/images/logos/ and add it to LOGOS in trust_logos.js.
