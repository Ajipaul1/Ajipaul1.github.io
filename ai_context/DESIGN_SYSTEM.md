...
## Wide-desktop tier (standard since 2026-09-02)

Mobile-first base + explicit wide tier on every page:
```css
@media (min-width:1440px){ .container{ max-width:1480px; } }
@media (min-width:1850px){ .container{ max-width:1720px; } }
```
(Blog hub scopes these per-section; homepage applies them globally; posts use 1200/1360px with a grid.) Desktop must use the screen — see RULES §9.

## Blog hub patterns (blog/index.html — REGENERATED, never hand-edited)

`blog/index.html` is generated by `scripts/build_bloghub.js` (see scripts/README.md). Hand edits WILL be lost on regeneration — change the generator instead. Architecture (researched against blog.google + blog.hubspot.com):
- Full-viewport wave-canvas hero (`.tap-new-hero.blog-hero`, height `min(88vh, 820px)`), display headline, category pills, honest stat row, bouncing scroll cue.
- Featured post card OVERLAPPING the hero bottom (`margin-top:-90px`).
- "Start Here" trio → one pillar explainer per service.
- Card grid: photo top (16:10, hover zoom), category accent bar (seo=green, erp=navy, web=orange, out=gold), tag/title/excerpt/date + read-time COMPUTED from the post's real word count.
- Category filter pills + client-side pagination (12/page) working together; ALL cards stay in the HTML for crawlers — pagination is presentation-only.
- Scroll-reveal: `.reveal`/`.in` classes applied BY JS (IntersectionObserver, `rootMargin: 0 0 -8%`), so no-JS sees everything; `prefers-reduced-motion` disables.
- Schema: CollectionPage + BreadcrumbList + full ItemList of every post.

## Modern blog post anatomy (the 4 reference posts)

Lead figure (no caption) → navy **Key Takeaways** box (3 bullets, `strong/em` forced `#fff`) → answer-first paragraph → sections with 1–2 captioned inline figures (`figcaption` mono, centered) → callout/checklist components → navy CTA box → author row. Figures: `max-height:420px; object-fit:cover`. Wide desktop (≥1280px): article container becomes `grid: minmax(0,780px) 300px`, right column = sticky "On this page" TOC (JS-built from h2s, scrollspy active state, CTA link at bottom); ≥1700px: 860px + 320px. Footer widens with it. Apply this exact anatomy to all future posts (reference scripts in `scripts/`).

## Screenshot verification gotchas (hard-won)

- Headless Chrome clamps its window to ~512px wide — a 390px screenshot lays out at 512 and crops, FAKING mobile overflow. Test narrow layouts at ≥560px or with real device emulation.
