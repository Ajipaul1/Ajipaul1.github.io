...
async function runAutopilot() {
  console.log("🚀 Starting SEO Autopilot Engine...");
  
  const db = readDb();
  const queue = db.prospects.filter(p => p.status === 'discovered');
  
  if (queue.length === 0) {
    console.log("No new prospects discovered in queue. Add prospects to db.json first!");
    return;
  }
  
  console.log(`Found ${queue.length} prospects in queue. Initiating browser automation...`);
  
  const browser = await chromium.launch({ headless: false }); // Set to false so you can see it work!
  
  for (const prospect of queue) {
    console.log(`\n--------------------------------------------`);
    console.log(`Processing: ${prospect.domain}`);
    ...
    try {
      ...
      console.log("Analyzing page form layout elements...");
      
      // Smart Heuristics to identify and fill input fields
      ...
      for (const input of inputs) {
        ...
        // Match message / pitch textareas
          ...
          const pitch = "Hello Editor,\n\nI read your blog and found your article on Core Web Vitals very helpful. We've published a detailed audit showing how local Kochi businesses can speed up LCP scores by 300%.\n\nI think your readers would find this highly valuable. Let me know if you would be open to guest featuring this on your site!\n\nBest,\nAji Paul\nTechAuditPros.com";
          await input.fill(pitch);
          console.log(`Filled Pitch field: name="${name}", id="${id}"`);
        }
      }
      ...
      if (submitBtn) {
        ...
        await page.waitForTimeout(5000); // Wait for navigation or success response
      } else {
        console.log("No submit button found. Saving as drafted screenshot.");
      }
      
      // Update prospect status in DB
      prospect.status = 'completed';
      
      // Append outreach logs
  ...
  await browser.close();
  console.log("\n--------------------------------------------");
  console.log("SEO Autopilot Run completed!");
}
